...
const Additions = ({ addition }) => {
	return (
		<div className="order-page__checkbox extra-page__checkbox">
			<input
				type="checkbox"
				className="order-page__custom-checkbox"
				id={addition.id}
				name={addition.id}
				value={addition.id}
			/>
			<label htmlFor={addition.id}>
...
const ExtraPage = () => {
	...
	const clearFrom =
		fromDate.length > 0 ? 'order-page__cross' : 'order-page__cross--hidden'
	const clearTo =
		toDate.length > 0 ? 'order-page__cross' : 'order-page__cross--hidden'
	...
	return (
		<section className="extra-page order-page">
			<h1 className="visually-hidden">
				Станица заказа - выбор дополнительных услуг
			</h1>
			<BurgerMenu />
			<div className="order-page__wrapper">
				...
				<div className=" order-page__container order-page__content">
					<div className="order-page__left-container">
						...
						<div className="order-page__date">
							...
							<label className="order-page__label extra-page__label">
								...
								></input>
								<button className={clearFrom} onClick={() => setFromDate('')}>
									<OrderCross />
								</button>
							</label>
							<label className="order-page__label extra-page__label">
								...
								></input>
								<button className={clearTo} onClick={() => setToDate('')}>
									<OrderCross />
								</button>
							</label>
						</div>
					...
					<div className="order-page__right-container">
						...
						<Link to="/order/final" className={buttonClass}>
							Итого
						</Link>
					</div>
				</div>
