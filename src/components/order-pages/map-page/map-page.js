...
const MapPage = () => {
	...
	const clearCity =
		city.length > 0 ? 'order-page__cross' : 'order-page__cross--hidden'
	const clearDelivery =
		delivery.length > 0 ? 'order-page__cross' : 'order-page__cross--hidden'
				...
				<div className=" order-page__container order-page__content">
					<div className="order-page__left-container">
						...
						<label className="order-page__label map-page__delivery">
							...
							></input>
							<button className={clearDelivery} onClick={() => setDelivery('')}>
								<OrderCross />
							</button>
						</label>
					...
					<div className="order-page__right-container">
						...
						<Link to="/order/model" className={buttonClass}>
							Выбрать модель
						</Link>
					</div>
				</div>
